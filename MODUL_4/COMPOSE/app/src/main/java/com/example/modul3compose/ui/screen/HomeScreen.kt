...
import com.example.modul3compose.viewmodel.GameViewModel
import com.example.modul3compose.viewmodel.GameViewModelFactory

@Composable
fun HomeScreen(
    navController: androidx.navigation.NavController,
    viewModel: GameViewModel = viewModel(factory = GameViewModelFactory("Hafiz Perdana"))
) {
        ...
        LazyColumn(modifier = Modifier.fillMaxSize().padding(p)) {
            item {
                ...
                LazyRow(contentPadding = PaddingValues(horizontal = 16.dp), horizontalArrangement = Arrangement.spacedBy(12.dp)) {
                    items(games) { game ->
                        Card(
                            onClick = { viewModel.onDetailClicked(game) },
            ...
            items(games) { game ->
                GameListItem(
                    game = game,
                    onDetail = { viewModel.onDetailClicked(game) },
...
@Composable
fun GameListItem(game: Game, onDetail: () -> Unit, onBrowser: (String) -> Unit) {
            ...
            Column(Modifier.padding(start = 12.dp).fillMaxHeight()) {
                ...
                Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.End) {
                    ...
                    Button(
                        onClick = { 
                            onBrowser(game.url)
                            ctx.startActivity(Intent(Intent.ACTION_VIEW, game.url.toUri())) 
                        }, 
                        colors = blue, 
