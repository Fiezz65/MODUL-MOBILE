...
fun HomeScreen(
    ...
    viewModel: AnimeViewModel = viewModel(factory = AnimeViewModelFactory("Hafiz Perdana"))
) {
        ...
        LazyColumn(modifier = Modifier.fillMaxSize().padding(p)) {
            item {
                ...
                LazyRow(contentPadding = PaddingValues(horizontal = 16.dp), horizontalArrangement = Arrangement.spacedBy(12.dp)) {
                    items(animes) { anime ->
                        Card(
                            onClick = { viewModel.onDetailClicked(anime) },
                            modifier = Modifier.size(200.dp, 120.dp),
                            shape = RoundedCornerShape(12.dp)
                        ) {
                            Image(painterResource(anime.imageRes), null, contentScale = ContentScale.Crop, modifier = Modifier.fillMaxSize())
                        }
                    }
                }
            ...
            items(animes) { anime ->
                AnimeListItem(
                    anime = anime,
                    onDetail = { viewModel.onDetailClicked(anime) },
                    onBrowser = { url -> viewModel.onExplicitIntentClicked(url) }
                )
            }
        }
    }
...
@Composable
fun AnimeListItem(anime: Anime, onDetail: () -> Unit, onBrowser: (String) -> Unit) {
    ...
    Card(
        ...
        Row(Modifier.padding(12.dp).fillMaxSize()) {
            Image(
                painterResource(anime.imageRes), null,
            ...
            Column(Modifier.padding(start = 12.dp).fillMaxHeight()) {
                Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) {
                    Text(anime.title, fontWeight = FontWeight.Bold, maxLines = 1, overflow = TextOverflow.Ellipsis, modifier = Modifier.weight(1f))
                ...
                Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.End) {
                    ...
                    Button(
                        onClick = { 
                            onBrowser(anime.url)
                            ctx.startActivity(Intent(Intent.ACTION_VIEW, anime.url.toUri()))
                        },
                        colors = blue, 
